...
public class Agent {
	...
	public Agent(int iniSugar, int vis, int met, Random rand, int[] loc,
			int preset, int size, int maxLife, int[] fertile) {
		sugar = iniSugar;
		ferSugar = sugar;
		vision = vis;
		meta = met;
	...
	public Agent(int iniSugar, int vis, int met, int age, int ferSu,
			Random rand, int[] loc, int preset, int size, int maxLife,
			int[] fertile) {
		sugar = iniSugar;
		ferSugar = ferSu;
		vision = vis;
	...
	public int[] findNext(int[][] map, Random rand) {
		...
		for (int i = 0; i < vision; i++) {
			int[][] potentials = { { loc[0] + i + 1, loc[1] },
					{ loc[0] - i - 1, loc[1] }, { loc[0], loc[1] + i + 1 },
					{ loc[0], loc[1] - i - 1 } };
			for (int[] potential : potentials) {
				if ((potential[0] >= 0 && potential[0] < map.length)
						&& (potential[1] >= 0 && potential[1] < map.length)) {
					if (map[potential[0]][potential[1]] > thisSugar
							&& map[potential[0]][potential[1]] != 5) {
						candidateC = 1;
		...
		if (candidateC > 1) {
			...
			for (int i = 0; i < candidateC - 1; i++) {
				...
				double thisDist = Math.pow(thisCand[0] - loc[0], 2)
						+ Math.pow(thisCand[1] - loc[1], 2);
	...
	public Integer mate(Agent thisAgent, int[][] map, Random rand,
			Map<Integer, Agent> babies) {
		if (thisAgent.gender != this.gender
				&& this.active()
				&& thisAgent.active()
				&& !this.hungry()
				&& !thisAgent.hungry()
				&& (thisAgent.hasEmptyPatch(map) != null || this
						.hasEmptyPatch(map) != null)) {
			if (!this.mated.contains(thisAgent.getIndex())
					&& !thisAgent.mated.contains(this.getIndex())) {
	...
	private List<Integer> hasEmptyPatch(int[][] map) {
		...
		for (int[] neighbor : neighbors) {
			if ((loc[0] + neighbor[0] >= 0 && loc[0] + neighbor[0] < map.length)
					&& (loc[1] + neighbor[1] >= 0 && loc[1] + neighbor[1] < map.length)
					&& map[loc[0] + neighbor[0]][loc[1] + neighbor[1]] != 5) {
				int newLoc = (loc[0] + neighbor[0]) * size + loc[1]
						+ neighbor[1];
				index.add(newLoc);
			}
		}
