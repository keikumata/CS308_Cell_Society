...
public class Forage extends Sim {
	...
	public Forage(int sim, int cellTypes, int size, int delay, int cellSides,
			List<Integer> params) {
	...
	public void initMap() {
		...
		homePh = new Pheromone(evapRate, diffRate, maxPhero, maxAnt, cellSides,
				size);
		homePh.maxPhero(nest[0], nest[1]);
		foodPh = new Pheromone(evapRate, diffRate, maxPhero, maxAnt, cellSides,
				size);
	}
			...
			while (it.hasNext()) {
				...
				if (ant.grow()) {
					...
					if (ant.hasFood) {
						...
						next = homePh.selectFrontLoc(ant.coor, fronts, rand,
								antMap);
						if (next == null) {
							next = homePh.selectSideLoc(ant.coor,
									ant.getSideNeighbors(), rand, antMap);
						}
					} else {
						homePh.topOffPhero(ant.coor[0], ant.coor[1], map);
						next = foodPh.selectFrontLoc(ant.coor, fronts, rand,
								antMap);
						if (next == null) {
							next = foodPh.selectSideLoc(ant.coor,
									ant.getSideNeighbors(), rand, antMap);
						}
					}
					if (next == null) {
