...
public class Fire extends Sim {
	...
	public Fire(int sim, int cellTypes, int size, int delay, int cellSides,
			List<Integer> params) {
	...
	private void checkFire(int row, int col, int[][] tempMap,
			List<Integer> burningTrees, SceneUpdater updater) {
		burnedTotal++;
		tempMap[row][col] = 2;
		...
		for (int[] neighbor : neighbors) {
			if ((row + neighbor[0] >= 0 && row + neighbor[0] < map.length)
					...
					&& map[row + neighbor[0]][col + neighbor[1]] == 0) {
				int treeIndex = (row + neighbor[0]) * map.length + col
						+ neighbor[1];
	...
	public HashMap<Integer, Integer> cellProportions() {
		...
		ret.put(0, (calculateTotal() - fireTotal - burnedTotal) * 100
				/ calculateTotal());
