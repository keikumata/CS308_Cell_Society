...
/**
 * 
 * @author Kei Yoshikoshi A class that creates a line chart that plots the
 *         different cell proportions per frame
 *
 */
public class AnimatedGraph {
	 ...
	 * 
	 * @param paramLabels
	 *            : HashMap that contains the index and the label
	 * @return HashMap that contains the index and XYChart.Series
	 */
	@SuppressWarnings("rawtypes")
	private HashMap<Integer, XYChart.Series> createAddSeries(
			HashMap<Integer, String> paramLabels) {
	...
	public Pane init() {
		...
		NumberAxis yAxis = new NumberAxis("Percentage of cells",
				MIN_PERCENTAGE, MAX_PERCENTAGE, TICK_UNIT);
	 ...
	 *            : the frame
	 * @param proportions
	 *            : HashMap that maps the index to the cell proportion at the
	 *            specific frame
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public void addData(int count, HashMap<Integer, Integer> proportions) {
		for (int i : proportions.keySet()) {
			seriesMap.get(i).getData()
					.add(new XYChart.Data(count, proportions.get(i)));
		}
	}
